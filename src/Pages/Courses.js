...
function Courses() {
  ...
  const [debouncedCodeSearchTerm, setDebouncedCodeSearchTerm] =
    useState(codeSearchTerm);
  ...
  const COURSES_URL =
    "https://ufcourseplanner.ryanlu.dev/data-api/rest/courses";
  const SECTIONS_URL =
    "https://ufcourseplanner.ryanlu.dev/data-api/rest/sections";
    ...
    while (hasNextPage) {
      try {
        ...
        if (Array.isArray(data.value)) {
          ...
          throw new Error(
            "Invalid data format received from the sections API."
          );
        }
  ...
  async function getTeacherRating(instructorName) {
    try {
      const teachers = await ratings.searchTeacher(instructorName);
      if (teachers.length === 0) {
        return null; // No teacher found
      }
      const teacherDetails = await ratings.getTeacher(teachers[0].id);
      return {
        avgRating: teacherDetails.avgRating,
        avgDifficulty: teacherDetails.avgDifficulty,
        numRatings: teacherDetails.numRatings,
      };
    } catch (error) {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      // Merge sections with course information and professor ratings
      const mergedSections = await Promise.all(
        sectionsData.map(async (section) => {
          const sectionCourseCode = section.course_code
            ? section.course_code.trim().toUpperCase()
            : "";
          const course = coursesMap.get(sectionCourseCode);

          // Fetch professor ratings
          const instructorName = section.instructor || "Unknown Instructor";
          let ratingData = null;

          // Check localStorage cache
          const cachedRatings = localStorage.getItem(
            `rating_${instructorName}`
          );
          if (cachedRatings) {
            ratingData = JSON.parse(cachedRatings);
          } else if (instructorName !== "Unknown Instructor") {
            ratingData = await getTeacherRating(instructorName);
            if (ratingData) {
...
*/
  ...
  const handleSignUp = (section) => {
    ...
    if (!isAlreadySignedUp) {
      ...
      localStorage.setItem(
        "signedUpSections",
        JSON.stringify(newSignedUpSections)
      );
    }
  };
  ...
  const handleRemoveSignedUpSection = (sectionId) => {
    ...
    localStorage.setItem(
      "signedUpSections",
      JSON.stringify(updatedSignedUpSections)
    );
  };
  ...
  const uniqueCourseCodes = useMemo(() => {
    ...
    const codesSet = new Set(
      allCourses.map((course) => course.code.trim().toUpperCase())
    );
  ...
  const filteredCodes = useMemo(() => {
    if (debouncedCodeSearchTerm.trim() === "" || uniqueCourseCodes.length === 0)
      return [];
    return uniqueCourseCodes
      .filter((code) =>
        code.includes(debouncedCodeSearchTerm.trim().toUpperCase())
      )
  ...
  useEffect(() => {
    ...
    const mergedFilteredSections = filteredSections.map((section) => {
      ...
      if (course) {
        ...
        console.warn(
          `No matching course found for course_code: "${section.course_code}" in section ${section.section_id}`
        );
        return {
          ...section,
          courseTitle: "Unknown Course",
                ...
                >
                  {filteredCodes.map((code) => (
                    <li
                      ...
                      role="option"
                      aria-selected={
                        code === codeSearchTerm.trim().toUpperCase()
                      }
                      tabIndex="0"
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                ...
                {currentSections.map((section) => (
                  <li key={section.section_id} className="section-item">
                    <div className="section-details">
                      <h3>
                        {section.courseTitle} - Section {section.section_id}
                      </h3>
                      <p>
                        <strong>Course Code:</strong> {section.course_code}
                      </p>
                      <p>
                        <strong>Department ID:</strong> {section.department_id}
                      </p>
                      <p>
                        <strong>Credits:</strong> {section.credits}
                      </p>
                      ...
                      {section.ratings && (
                        <p>
                          <strong>RateMyProfessor:</strong>
                          {section.ratings.avgRating} / 5 (Difficulty:{" "}
                          {section.ratings.avgDifficulty},
                          {section.ratings.numRatings} Ratings)
                        </p>
                      )}
                      <p>
                        <strong>Room:</strong> {section.room}
                      </p>
                      <p>
                        <strong>Meeting Times:</strong> {section.meeting_times}
                      </p>
                ...
                {signedUpSections.map((section) => (
                  <li key={section.section_id} className="signed-up-item">
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleRemoveSignedUpSection(section.section_id)
                      }
